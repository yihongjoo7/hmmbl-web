...
본 프로젝트는 **① 네이티브 앱 WebView 내 실행**과 **② 브라우저를 통한 순수 Web 접근**을 모두 지원한다.  
이 문서의 모든 표준은 두 환경을 함께 고려해 작성되었으며, 환경별로 규칙이 달라지는 항목은 각 장에서 별도로 명시한다.

### 1-1. 실행 환경 구분

| 구분 | WebView | 순수 Web |
|---|---|---|
| 접근 경로 | 네이티브 앱 내부에서 진입 | 브라우저로 URL 직접 접근 |
| 대응 라우트 그룹 | `app/(protected)/` (webview SSO 부트스트랩 필요) | `app/(public)/` (부트스트랩 없이 접근 가능) |
| 인증 방식 | Bridge를 통한 네이티브 토큰 수신, DPoP 기반 API 인증 | 별도 인증 없음 또는 웹 자체 로그인 플로우 |
| 로케일 결정 | 네이티브 앱이 결정 → Bridge 이벤트로 전달, `HPOINT_LOCALE` 쿠키 동기화 | 브라우저 언어(`Accept-Language`) 또는 URL 파라미터 기반 |
| SEO / 초기 로딩 성능 | 고려 불필요(검색엔진 노출 대상 아님) | 고려 필요(검색엔진 인덱싱, 초기 렌더 속도) |
| 네이티브 기능 연동 | `window.bridge`(카메라, 푸시, 네이티브 로그인 등) 사용 가능 | 네이티브 기능 접근 불가 — 기능 분기 또는 대체 UI 필요 |

새 화면을 추가할 때는 위 표를 기준으로 어떤 환경을 대상으로 하는지 먼저 판단하고, 그에 맞는 라우트 그룹과 렌더링 전략(7장)을 선택한다.  
두 환경 모두에서 접근 가능한 화면(예: `(public)`의 일부 화면)은 `window.bridge` 존재 여부를 런타임에 분기 처리해 네이티브 기능 유무에 관계없이 동작하도록 구현한다.
...
1-1절에서 정의한 두 실행 환경(WebView / 순수 Web)에 따라 렌더링 전략이 달라진다.  
`(protected)` 등 WebView 전용 화면은 SSR 데이터 페칭 없이 클라이언트 중심으로 동작하고, `(public)` 중 검색엔진 노출·초기 로딩 속도가 중요한 순수 Web 화면(랜딩, 소개 등)은 Server Component 기반 SSR을 적극 활용한다.  
아래는 실제 코드베이스에서 사용 중인 React 기능과, 두 환경에 따라 달라지는 적용 원칙이다.

### 7-1. 'use client' 사용 원칙

- **WebView 화면(`(protected)` 등)**: `page.tsx`를 제외한 사실상 모든 컴포넌트는 `'use client'`로 선언한다(현재 275개 파일 적용). `app/**/page.tsx`는 서버 컴포넌트로 얇게 유지하고, 실제 로직·UI는 `'use client'`로 선언된 `features/[domain]/.../[Screen]Page.tsx`에 위임하는 thin wrapper 패턴을 따른다. WebView는 검색엔진에 노출되지 않으므로 SSR로 얻는 이점이 없어 클라이언트 렌더링을 기본으로 한다.
...
- **순수 Web 화면(`(public)` 중 SEO·초기 로딩이 중요한 화면)**: 정적/준정적 콘텐츠(랜딩, 소개, 이용약관 등)는 `'use client'` 없이 서버 컴포넌트로 유지해 서버에서 완성된 HTML을 내려준다. 사용자 인터랙션이 필요한 부분만 하위 클라이언트 컴포넌트로 분리한다(Server Component가 Client Component를 감싸는 구조).

### 7-2. Server Components 사용 범위

- **WebView 화면**: 서버 컴포넌트는 `page.tsx` 진입점 용도로만 최소 사용한다. 서버 컴포넌트 내부에서 데이터 fetch·DB 접근을 직접 수행하지 않는다. 데이터 페칭이 필요한 화면은 예외 없이 클라이언트 컴포넌트로 작성하고, `@tanstack/react-query`를 통해 데이터를 가져온다.
- **순수 Web 화면**: SEO에 필요한 메타데이터(`generateMetadata`)나 초기 노출 콘텐츠는 서버 컴포넌트에서 직접 fetch해 SSR로 내려준다. 이후 사용자 인터랙션(좋아요, 폼 제출 등)이 필요한 부분에서만 클라이언트 컴포넌트 + React Query를 사용한다.
...
- 이 패턴은 **WebView 전용 이슈**를 해결하기 위한 것이다. WebView 화면은 네이티브 앱이 결정한 로케일을 쿠키(`HPOINT_LOCALE`)로 전달받는데, 서버가 첫 렌더 시점에 이 쿠키 상태를 알 수 없어 서버·클라이언트 렌더링 결과가 달라질 수 있다(hydration mismatch). 이를 막기 위해 로케일에 의존하는 Provider를 `next/dynamic({ ssr: false })`로 로드해 클라이언트에서만 렌더링한다.
...
- **순수 Web 화면**은 쿠키가 아닌 URL 세그먼트나 `Accept-Language` 헤더로 로케일을 서버에서 바로 판별할 수 있으므로 이 워크어라운드가 필요 없다. 순수 Web 전용 컴포넌트에 `ssr:false`를 습관적으로 적용하지 않는다 — SSR을 끄면 SEO·초기 로딩 이점이 사라진다.

### 7-4. useEffect

- 용도를 Bridge 이벤트 구독, `IntersectionObserver` 등 DOM/외부 API 연동처럼 정리(cleanup)가 필요한 부수효과로 한정한다.
- 구독형 effect는 반드시 cleanup 함수를 반환한다. 의존성 배열은 ESLint `exhaustive-deps` 규칙을 따르고 임의로 생략하지 않는다.
  ...
  ```ts
  // features/shared/hooks/useBridgeEvent.ts
  export function useBridgeEvent<T>(eventName: BridgeEventName, handler: (data: T) => void): void {
    useEffect(() => {
...
| `useTransition` / `useDeferredValue` | 미사용 | WebView 화면은 필요성이 낮음. 순수 Web에서 대량 리스트 필터링·검색처럼 입력 지연이 체감되는 인터랙션이 생기면 도입을 검토하되, 팀 리뷰를 거친다 |
...
- 인증 방식은 실행 환경에 따라 다르다.

  | 환경 | 인증 방식 | 401 처리 |
  |---|---|---|
  | WebView(`(protected)`) | Bridge로 수신한 네이티브 토큰 기반 DPoP 인증 | 자동 갱신 후 1회 재시도, 실패 시 로그아웃 처리 |
  | 순수 Web(`(public)`) | 인증 불필요 또는 별도 웹 로그인 플로우(도메인별 정의) | 화면 단에서 로그인 유도 UI로 처리, WebView 전용 Bridge 재시도 로직에 의존하지 않음 |

- `window.bridge`가 없는 순수 Web 환경에서는 Bridge 기반 인증·기능 호출을 시도하지 않도록 호출 전 존재 여부를 확인한다.

---

## 10. 다국어(i18n) 표준

- **WebView 화면**: `next-intl`의 `NextIntlClientProvider`만 사용하며, 빌드 플러그인·URL 기반 라우팅은 사용하지 않는다. 로케일은 네이티브 앱이 결정해 Bridge 이벤트로 전달하고, `HPOINT_LOCALE` 쿠키와 `LocaleProvider`(7-3절)를 통해서만 전환한다. 화면 단에서 로케일을 임의로 판단하지 않는다.
- **순수 Web 화면**: 검색엔진이 언어별 페이지를 개별 인덱싱할 수 있도록 URL 기반 로케일 라우팅(예: `/en/...`, `/ko/...`)을 사용하는 것을 권장하며, 최초 로케일은 `Accept-Language` 헤더 또는 URL 세그먼트로 서버에서 판별한다. WebView 전용인 쿠키·Bridge 기반 로케일 결정 방식에 의존하지 않는다.
- 번역 리소스는 두 환경 모두 `messages/*.json`에 도메인별 키로 관리하며, 최상위 키는 도메인명, 하위 키는 camelCase를 따른다.
...
| 구분 | 담당 | 허용 | 금지 |
|---|---|---|---|
| `*View.tsx` | 퍼블리셔(전담) | props 기반 렌더링, Tailwind 스타일링, `next/image` 사용, `components/common/ui/` import, 목업 미리보기 페이지에서만 `useState`로 인터랙션 시뮬레이션 | `useState`/`useEffect`, `fetch`/`apiClient`/React Query 직접 호출, `useRouter`/`useParams`, 하드코딩 색상값, `window.bridge` 직접 접근, Zustand Store 직접 import |
