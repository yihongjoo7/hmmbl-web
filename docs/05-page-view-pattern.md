...
> 레퍼런스 확인: `https://localhost:3001/dev/ref/publisher`
...
실제 코드는 `https://localhost:3001/dev/ref/publisher`에서 확인합니다.
...
4. **dev/pub에 등록** — 작업 중인 화면을 퍼블리셔 미리보기(`/dev/pub`)에 등록해 확인합니다. ([08-ui-catalog-contribution.md](./08-ui-catalog-contribution.md) 참고)
...
- [ ] 로딩 스켈레톤이 실제 레이아웃과 형태가 유사한가?
- [ ] 빈 상태 문구가 기획서와 일치하는가?
- [ ] 에러 상태 문구가 기획서와 일치하는가?
- [ ] 이미지에 `next/image`를 사용했는가? (`<img>` 직접 사용 금지)
- [ ] 썸네일 비율이 깨지지 않는가? (`object-cover` / `object-contain`)
- [ ] 긴 텍스트에서 `truncate` / `line-clamp`가 정상 동작하는가?
- [ ] 모바일 하단 safe-area(`pb-safe`)가 홈 인디케이터를 가리지 않는가?
- [ ] 터치 피드백(`active:bg-*`)이 모든 클릭 요소에 적용되어 있는가?
- [ ] 하드코딩 색상값 없이 Tailwind 토큰 클래스를 사용했는가?
- [ ] `useState` / `useEffect` / `apiClient`를 사용하지 않았는가?
- [ ] 콜백 props가 optional(`?`)로 선언되어 있는가?
