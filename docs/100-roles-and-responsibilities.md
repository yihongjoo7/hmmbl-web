...
> 이 문서는 hpoint-mobile 프로젝트에서 퍼블리셔와 개발자의 역할 경계, 책임 범위, 기대 수준을 명확하게 정의합니다.

---

## 1. 개요

hpoint-mobile은 **Page / View 분리 패턴**을 기반으로 UI 담당(퍼블리셔)과 로직 담당(개발자)을 분리합니다. 두 역할은 독립적으로 작업하되, PR 리뷰와 Props 인터페이스 협의를 통해 협력합니다.
...
퍼블리셔는 **UI 표현층(View)**을 전담합니다. 데이터가 어디서 오는지, 어떻게 저장되는지는 관여하지 않습니다. props로 받은 데이터를 화면에 올바르게 렌더링하는 것이 전부입니다.
...
- antd-mobile 컴포넌트 import (단, `'use client'` 선언 필수)
...
개발자는 **비즈니스 로직, 데이터 페칭, 인프라, 보안**을 전담합니다. View에 올바른 데이터를 전달하고, 앱이 안전하고 안정적으로 동작하도록 보장합니다.
...
| `package.json` | **전담** | 의존성 추가·업그레이드 (antd-mobile 포함) |
...
`lib/` 레이어는 bridge·Zustand·React에 의존하지 않아야 합니다. 이 원칙을 유지하는 것은 개발자의 책임입니다.
...
| 영역 | 내용 |
|---|---|
| CSP | 프로덕션 nonce 기반 CSP 유지. 인라인 스크립트·스타일 금지 |
...
- antd-mobile 사용 파일에 `'use client'` 선언 여부
- 콜백 props의 `optional(?)` 선언 여부
- 신규 antd-mobile 컴포넌트 도입 시 번들 크기 영향
- 신규 이미지 도메인 요청 시 `next.config.mjs` 업데이트

### 3-9. 브랜치 및 배포 책임

- `develop` → `main` 머지는 개발자가 담당
- 퍼블리셔 브랜치는 `feature/pub/` 접두어, 개발자 브랜치는 `feature/dev/` 접두어
...
`styles/globals.css`의 CSS 변수 값을 변경할 때는 사전 협의 후 진행합니다. 색상 토큰 변경은 모든 화면에 영향을 주므로 두 역할 모두의 검토가 필요합니다.

### antd-mobile 도입 및 업그레이드

퍼블리셔가 도입 필요성을 제안하고, 개발자가 `package.json`을 수정하고 번들 영향을 검토합니다. 업그레이드 후 퍼블리셔와 함께 UI를 검증합니다.

### IA 데이터 (`app/dev/ia/_data/ia.ts`)

화면 추가 시 두 역할 모두 수정할 수 있습니다. 실제 라우트 파일(`app/**/page.tsx`) 생성은 개발자가 담당합니다.
...
| `main` 브랜치 직접 push | **양쪽 모두 금지** | **양쪽 모두 금지** |
