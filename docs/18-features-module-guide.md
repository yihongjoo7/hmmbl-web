...
| `features/menu/` | 레거시 메뉴 | (IA 미등록, `06-ia-navigator.md` §3 참고) |
...
도메인 간 직접 import는 금지합니다. `features/earn/`에서 `features/my/`를 import하지 않습니다. 공유가 필요하면 `features/shared/`로 이동합니다.
...
Bridge 이벤트를 구독하는 기본 훅입니다. 컴포넌트 unmount 시 자동으로 구독 해제됩니다.
...
Android 백 버튼 이벤트 전용 훅입니다. `useBridgeEvent('appBackPressed', ...)`의 편의 래퍼입니다.
...
카메라/갤러리에서 이미지를 캡처해 `File` 객체로 반환합니다. 웹뷰가 아닌 환경에서는 `<input type="file">`로 폴백합니다.
...
GPS 위치 요청 훅입니다. 웹뷰이면 네이티브 GPS, 아니면 `navigator.geolocation`으로 자동 분기합니다.
...
햅틱 피드백 훅입니다. 웹뷰가 아닌 환경에서는 `navigator.vibrate`로 폴백합니다.
...
클립보드 복사 훅입니다. bridge → `navigator.clipboard` → `execCommand` 순서로 3단 폴백합니다. 성공 시 햅틱 피드백(light)이 자동 실행됩니다.
...
**`useLocaleChange()`** — 자세한 내용은 `16-i18n-guide.md` 참고

```ts
const locale = useLocaleChange(); // 'ko' | 'en' | 'ja' | 'zh'
```

### 파일 업로드 훅

**`useFileUpload(kind)`** — 자세한 내용은 `15-api-client.md` 참고
...
파일 관련 API 서비스입니다. `lib/api/fileUploadClient`와 `apiClient`를 조합해 features 레이어에서 사용하기 편리한 인터페이스를 제공합니다.
