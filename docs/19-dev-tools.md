...
`https://localhost:3001/dev`에 접속하면 개발 도구 허브를 볼 수 있습니다. 개발 환경 전용이며 프로덕션에서는 접근을 차단해야 합니다.
...
| `/dev/auth` | 인증 디버그 | 토큰 상태, 사용자 정보, AuthStore 확인(§4) |
...
IA 구조가 정의된 중앙 데이터 파일입니다. `_data/` 폴더는 Next.js 라우팅에서 제외됩니다.
...
로그인 없이 개발할 때 또는 인증 흐름을 디버깅할 때 사용합니다. 마운트 시 `appAuthCode` 이벤트를 구독하고 `window.bridge?.requestAuthCode()`를 호출해 자동으로 인증을 시도합니다.
...
새 공통 컴포넌트를 추가할 때 `/dev/ui` 등록 방법은 `08-ui-catalog-contribution.md`를 참고하세요.
...
`npm run dev`(포트 3001)는 HTTPS로 실행됩니다. 인증서 경로는 `next.config.mjs`가 아니라 **`package.json`의 `dev` 스크립트**에 지정되어 있습니다.

```json
"dev": "next dev --experimental-https --experimental-https-key ../keyfile/local-key.pem --experimental-https-cert ../keyfile/local-cert.pem --port 3001 --hostname 0.0.0.0"
```

즉 인증서 파일은 프로젝트 루트가 아니라 **상위 디렉토리의 `keyfile/` 폴더**(`../keyfile/local-key.pem`, `../keyfile/local-cert.pem`)에 있어야 하며, 저장소에는 포함되지 않습니다(각자 준비). 인증서가 없거나 만료된 경우 mkcert 등으로 재발급해 해당 경로에 배치합니다.

```bash
# mkcert 설치 후 (macOS: brew install mkcert)
mkcert -install
mkcert -key-file ../keyfile/local-key.pem -cert-file ../keyfile/local-cert.pem localhost 127.0.0.1
```

인증서 없이 빠르게 확인하려면 `npm run dev:http`(포트 3000, HTTP)를 사용할 수 있습니다.
...
`next.config.mjs`의 `allowedDevOrigins`도 확인합니다. 기본값은 `['localhost', '127.0.0.1']`입니다.

모바일 브라우저에서 `https://192.168.1.100:3001`으로 접속합니다(HTTPS, `npm run dev` 기준).
