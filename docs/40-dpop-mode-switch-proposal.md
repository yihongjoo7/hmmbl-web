# 30. 웹뷰 DPoP 처리방식 이중화 (.env 전환) — 설계 제안

> ⚠️ **현행화 안내(소스 기준):** 이 문서는 **2-Lite(proof만 네이티브 위임)를 권고**하며, 현재 구현도 2-Lite 방식으로 유지되고 있습니다.  
> 실제 구현 기준 최신 문서는 `32-auth-dpop-internals.md` 7장·`08-bridge-guide.md` 9장·`06-api-client.md` 1.1절입니다.  
> 아래 본문의 설계는 4장~5장의 `bridge.createDpopProof`/`requestNativeToken` 설계를 그대로 채택했습니다(5.2절의 `tokenReceived` 401 재사용 부분은 7장 대신 `lib/auth/interceptor.ts`에 반영).
>
...
서버 `create_access_token()`은 `sub/email/role/iat/exp/jti`만 담고 **`cnf`(키 지문) 클레임이 없다.**  
`verify_dpop_proof()`도 **`ath`(액세스 토큰 해시)와 `jkt`(키 thumbprint) 매칭을 검사하지 않는다.**  
proof는 "유효한 P-256 키로 올바른 htm/htu/iat/jti를 서명했는가"만 본다.

> **결론:** **어느 키로 서명한 proof든 동일한 액세스 토큰에 유효하다.**  
> 따라서 웹 키(Mode 1)와 네이티브 키(Mode 2)는 서버 관점에서 교환 가능하며, **모드 전환은 순수하게 클라이언트(웹+네이티브)만의 문제다.  
> 서버 변경이 필요 없다.**
...
"네이티브 위임"은 강도에 따라 두 가지로 갈린다.  
이미 `20100` 문서가 이 둘을 분석했고, **전면 위임(2-Full)은 비권장**, **sign 전용 제한 위임은 권고**로 결론냈다.
...
- 20100 5장 권고("KeyStore 연동을 **sign 용도로만 극도로 제한적으로** 추가")와 일치.
...
> **권고: 2-Lite.**  
> 이하 설계는 2-Lite 기준이며, 2-Full이 필요하면 별도 검토.
...
현행은 `buildHeaders()`가 `createDPoPProof`를 직접 부르고, 토큰은 `configureApiClient({getToken})`로 주입된다.  
이 **기존 주입 지점을 그대로 활용**해 모드별 구현만 갈아끼운다.  
(lib 순수성·의존성 역전 규칙 유지.)
...
> 토큰 보관처는 두 모드 모두 `useAuthStore._accessToken`(메모리)로 일원화 → `buildHeaders`의 `getToken`은 그대로.  
> 차이는 "그 토큰을 누가 채우는가"뿐(Mode1=웹 교환, Mode2=네이티브 푸시).
...
토큰이 키에 바인딩되지 않으므로(2장) 네이티브 키 proof가 그대로 통과.  
**무변경.**

> (선택·별개 과제) 보안을 더 높이려면 `cnf.jkt` 바인딩 + proof `ath` 검증을 도입할 수 있으나, 이는 두 모드 **모두** 키 일관성을 강제하게 되어 전환 설계와 독립적으로 다뤄야 한다.  
> 본 제안 범위 밖.
